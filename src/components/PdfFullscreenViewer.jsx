  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // 所有頁面」包成一個函式，用 Worker 模式整段跑一次，任何一步失败就整個放棄、改用
        // 「不用 Worker」模式（在主執行緒直接跑，效能較差但相容性好很多）重新跑一次。
  ...
  return (
    <div className="pdf-fullscreen-overlay">
      ...
      <div className="pdf-fullscreen-body">
        ...
        {/* 这个 div 永远保持空白、不透过 JSX 渲染任何子节点，这样 React 认定它「没有子节点需要管」，
            之后 PDF.js 用 appendChild 手动塞进去的 canvas 元素就不会跟 React 自己的 DOM
            更新逻辑互相打架，避免 removeChild 报错整个 App 崩溃 */}
