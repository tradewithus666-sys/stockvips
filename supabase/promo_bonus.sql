alter table public.payment_intents add column if not exists credit_amount numeric(12,2);

create or replace function public.create_recharge_intent(p_amount numeric, p_address text, p_credit_amount numeric default null)
...
begin
  ...
  insert into public.payment_intents (member_id, product_id, duration, amount, address, kind, last_scanned_block, credit_amount)
  values (auth.uid(), null, 'recharge', p_amount, p_address, 'recharge', v_block, coalesce(p_credit_amount, p_amount))
...
begin
  ...
  if not (v_body ? 'error') and (v_body ? 'result') then
    ...
    loop
      ...
      then
        v_found := true;
        v_credit := coalesce(v_intent.credit_amount, v_intent.amount);
        if v_intent.kind = 'recharge' then
          update public.profiles set balance = balance + v_credit where id = v_intent.member_id;
          insert into public.wallet_tx (member_id, amount, status, tx_hash) values (v_intent.member_id, v_credit, 'success', v_log->>'transactionHash');
        else
  ...
  loop
    ...
    then
      ...
      if v_amount >= v_intent.amount * 0.99 then
        ...
        if v_intent.kind = 'recharge' then
          update public.profiles set balance = balance + v_credit where id = v_intent.member_id;
          insert into public.wallet_tx (member_id, amount, status, tx_hash) values (v_intent.member_id, v_credit, 'success', p_tx_hash);
        else
          select * into v_product from public.products where id = v_intent.product_id;
          v_days := case v_intent.duration when 'month' then 30 when 'quarter' then 90 when 'year' then 365 else null end;
          select expires_at into v_existing_expiry from public.permissions
